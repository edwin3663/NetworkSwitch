...
int SpiReceivePackets(INTC *IntcInstancePtr, XSpi *SpiInstancePtr, u16 SpiDeviceId, u16 SpiIntrId)
{
	...
	// Wait for a packet
	do {
		SpiTransferData(SpiInstancePtr, CommandBuffer[10], ReadBuffer, CommandLength[10]);
	} while(ReadBuffer[2] == 0);

	// Write to ERXRDPT
	SpiTransferData(SpiInstancePtr, CommandBuffer[11], NULL, CommandLength[11]);

	u8 ReadPckCommand[9] = {0x2C, 0x00 };
	SpiTransferData(SpiInstancePtr, ReadPckCommand, ReadBuffer, 9);

	NextPacketPtr = (ReadBuffer[2] << 2) + ReadBuffer[1];
	u16 FrameSize = (ReadBuffer[4] << 2) + ReadBuffer[3];
	xil_printf("Packet Size %d\r\n", FrameSize);

	u8 *FrameCommand = calloc( FrameSize + 1, sizeof(u8));
	u8 *FrameBuffer = calloc( FrameSize + 1, sizeof(u8));
	FrameCommand[0] = 0x2C;

	SpiTransferData(SpiInstancePtr, FrameCommand, FrameBuffer, FrameSize + 1);
